...
def create_trade_signal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(1, len(df)):
        if df.at[i - 1, "5_day_MA"] < df.at[i - 1, "8_day_MA"] and df.at[i, "5_day_MA"] >= df.at[i, "8_day_MA"]:
            df.at[i, "signal"] = "buy"
        elif df.at[i - 1, "5_day_MA"] > df.at[i - 1, "8_day_MA"] and df.at[i, "5_day_MA"] <= df.at[i, "8_day_MA"]:
            df.at[i, "signal"] = "sell"
        else:
            df.at[i, "signal"] = "hold"
