...
def get_chart(scripts, start, end):
    ...
    for scriptid in scripts:
        ...
        new_row = pd.DataFrame({'Script': [scriptid], 'CMP': [cmp], 'Date': [start], 'before_5_yr': [before_5_yr], 'five_years_start': [five_years_start], 'before_10_yr': [before_10_yr], 'ten_years_start': [ten_years_start], 'return_5_yrs': [return_5_yrs], 'return_6_to_10_yrs': [return_6_to_10_yrs]})

        # Concatenate the two DataFrames along the rows (axis=0)
        df_new = pd.concat([df_new, new_row], axis=0, ignore_index=True)

    df_new.to_excel("research/stock_returns.xlsx")
    return df_new
